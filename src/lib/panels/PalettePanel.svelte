...
<section class="p-8 rounded-xl bg-[var(--ui-card)] border border-[var(--ui-border)] shadow-xl h-full">
    <div class="flex flex-col xl:flex-row items-center justify-between gap-4 mb-6">
        <h2 class="text-xs font-black uppercase tracking-widest text-[var(--ui-text-muted)] shrink-0 self-start xl:self-center">Palette</h2>

        <div class="flex flex-wrap gap-2 items-center justify-end w-full xl:w-auto">
            <select bind:value={engine.genAxis} class="bg-[var(--ui-bg)] border border-[var(--ui-border)] rounded-lg px-3 py-1.5 text-xs font-bold uppercase outline-none cursor-pointer transition-all focus:border-[var(--current-color)] flex-1 xl:flex-none min-w-[140px]">
            ...
            {#if !engine.isHarmonyMode}
                <div class="flex items-center gap-1 bg-[var(--ui-bg)] border border-[var(--ui-border)] rounded-lg p-1">
                    <button onclick={removeRow} class="p-1.5 hover:bg-[var(--ui-card)] rounded-md transition-colors text-[var(--ui-text-muted)] hover:text-[var(--current-color)]" title="Remove Row" aria-label="Decrease steps">
                        <ListMinus class="w-4 h-4" />
                    </button>
                    <button onclick={addRow} class="p-1.5 hover:bg-[var(--ui-card)] rounded-md transition-colors text-[var(--ui-text-muted)] hover:text-[var(--current-color)]" title="Add Row" aria-label="Increase steps">
