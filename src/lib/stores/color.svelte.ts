...
export class ColorStore {
  ...
  get l() { return this.#current.l; }
  set l(v: number) { this.#current.l = v; }

  get c() { return this.#current.c; }
  set c(v: number) { this.#current.c = v; }

  get h() { return this.#current.h ?? 0; }
  set h(v: number) { this.#current.h = v; }

  get alpha() { return this.#current.alpha ?? 1; }
  set alpha(v: number) { this.#current.alpha = v; }
  ...
  get isOutOfGamut() { return this.#isOutOfGamut; }
  ...
  lab = $derived.by(() => {
    ...
    return formatLab(labColor.l ?? 0, labColor.a ?? 0, labColor.b ?? 0, this.alpha, this.#precisionMode());
  });

  oklab = $derived.by(() => {
    const oklabColor = toOklab(this.#current);
    if (!oklabColor) return 'oklab(0 0 0)';
    return formatOklab(oklabColor.l ?? 0, oklabColor.a ?? 0, oklabColor.b ?? 0, this.alpha, this.#precisionMode());
  });
