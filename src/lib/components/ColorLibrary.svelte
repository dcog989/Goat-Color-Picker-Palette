...
<div class="h-full flex flex-col">
    {#if !forceExpanded}
        <div class="flex items-center justify-between mb-4">
            ...
            <button
                ...
                class="text-xs font-bold uppercase opacity-40 hover:opacity-100 transition-opacity">
    ...
    {#if shouldShowContent}
        <div
            class="flex-1 flex flex-col bg-[var(--ui-bg)] border border-[var(--ui-border)] p-4 md:p-6 overflow-hidden rounded-xl">
            <input
                bind:this={searchInput}
                type="text"
                bind:value={searchQuery}
                placeholder="Search 30,000+ colors..."
                class="w-full p-4 mb-4 bg-[var(--ui-card)] border border-[var(--ui-border)] font-mono text-sm outline-none focus:ring-2 focus:ring-brand/20 flex-shrink-0 shadow-inner rounded-md" />

            {#if isLoading}
                <div class="flex-1 flex items-center justify-center">
                    <div class="text-center">
                        <div
                            class="w-12 h-12 border-4 border-[var(--ui-border)] border-t-[var(--current-color)] rounded-full animate-spin mx-auto mb-4">
                        </div>
                ...
                <div
                    ...
                    {#if filteredColors.length > 0}
                        <div
                            ...
                            class="w-full absolute top-0 left-0 pointer-events-none">
                        </div>
                        ...
                        <div
                            ...
                            {#each visibleItems as color (color.hex + color.name)}
                                <button
                                    ...
                                    class="w-full flex items-center gap-4 p-2 h-[56px] hover:bg-[var(--ui-card)] transition-colors group box-border mb-1 text-left rounded-md">
                                    <div
                                        class="w-10 h-10 border border-white/10 shadow-sm group-hover:scale-110 transition-transform flex-shrink-0 rounded-sm [background:var(--item-bg)]"
                                        style:--item-bg={color.hex}
                                        title={colorStore.formatColor(color.hex)}>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div
                                            class="text-sm font-bold truncate text-[var(--ui-text)]">
                                            {color.name}
                                        </div>
                                        <div
