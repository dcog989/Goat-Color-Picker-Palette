<script lang="ts">
    ...
    let blockSize = $derived(() => {
        ...
        if (letterCount <= 8) return "w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 text-lg md:text-xl lg:text-2xl";
        if (letterCount <= 12) return "w-7 h-7 md:w-9 md:h-9 lg:w-11 lg:h-11 text-base md:text-lg lg:text-xl";
        if (letterCount <= 16) return "w-6 h-6 md:w-8 md:h-8 lg:w-10 lg:h-10 text-sm md:text-base lg:text-lg";
...
{#if useTicker()}
    ...
    <div class="overflow-hidden w-full max-w-[60vw]">
        <div class="flex animate-bounce whitespace-nowrap" style="gap: 0.5rem;">
            {#each name.toUpperCase().split("") as letter, i}
                {#if letter === " "}
                    <div class="w-3 md:w-5 transition-all duration-1000"></div>
                {:else}
                    {@const style = getBlockStyle(i)}
                    <span class="inline-flex items-center justify-center {blockSize} font-black rounded-sm shadow-md transition-all duration-1000 hover:scale-110 hover:rotate-2 cursor-default select-none [background:var(--letter-bg)] [color:var(--letter-text)]" style:--letter-bg={style.bgColor} style:--letter-text={style.textColor}>
                        {letter}
                    </span>
                {/if}
            {/each}
